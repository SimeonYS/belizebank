...
class BbelizebankPipeline:

    # Database setup
    conn = sqlite3.connect('belizebank.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `belizebank`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM belizebank WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO belizebank (date, title, link, content)"
